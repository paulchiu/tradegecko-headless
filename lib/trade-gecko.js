...
exports.publishVariantOnChannel = async function(
  browserPage,
  variantId,
  channelId
) {
  // Prepare request
  const uri = `${RESOURCES_URI}/variants/${variantId}/channels`;
  const payload = { channel: { channel_id: channelId } };

  // Inject request
  await browserPage.exposeFunction('tgContext', () => {
    return {
      uri,
      payload,
    };
  ...
  const result = await browserPage.evaluate(async () => {
    const {uri, payload} = await window.tgContext();
