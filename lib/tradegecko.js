...
exports.resourceCount = async function(browserPage, resource) {
  ...
  const headers = await exports.fetch(browserPage, "GET", endpoint, null, true);

  for (const headerKey in headers) {
    const header = [...headers[headerKey]];

    if (header[0] === 'x-pagination') {
      const page = JSON.parse(header[1]);
      return parseInt(page['total_records']);
    }
  }

  console.error("Unexpected response header");
  return "Error";
};
 ...
 *
 * @todo Implement serialise method for fetch response that can cross browserPage boundary
 *       currently returning just the response doesn't result in anything usable
 * @param {puppeteer.Page} browserPage
 * @param {string} method
 ...
 */
exports.fetch = async function(browserPage, method, endpoint, body, returnHeaders = false) {
  // Prepare request
  ...
  return await browserPage.evaluate(
    async (method, uri, body, returnHeaders) => {
      ...
      if (returnHeaders) {
        ...
        return `${response.status} ${response.statusText} ${method} ${uri}`;
      } else if (response.ok) {
