...
exports.downloadResourceList = async function(params) {
  ...
  const fileFormat = supportedFormats.indexOf(format) > -1 ? format : "json";
  ...
  let resources = await tradeGecko.getPagedResources(
    browserPage,
  ...
  if (fileFormat === "json") {
    ...
    resources = json2csvParser.parse(resources);
  }
...
exports.fetchWithResourceList = async function(params) {
  ...
  const data = JSON.parse(fs.readFileSync(listFile));
  ...
  logger(`Performing fetches ...`);
  const responses = [];
  const bar = new progress.Bar({}, progress.Presets.shades_classic);
  bar.start(stopPoint, index);
  ...
  logger(`Saving responses to file ...`);
...
/**
 * @typedef {Object} CountResourcesParams
 * @property {string} username
 * @property {string} password
 * @property {string} resource
 * @property {string} verbosity
 */
/**
 * @param {CountResourcesParams} params
 */
exports.countResources = async function(params) {
  const { username, password, resource, verbosity } = params;
  const logger = log.makeLogger(verbosity);
  ...
  logger(`Getting ${resource} count ...`, "v");
  const count = await tradeGecko.resourceCount(browserPage, resource);

  if (verbosity && verbosity.length > 0) {
    logger(`Total records: ${count}`);
  } else {
    logger(count);
  }
